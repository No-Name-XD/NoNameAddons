...
export function InDungeon() {
    try {
        ...
        ChatLib.chat(`&c&lNoNameAddons &8» &rError in checkInDungeon: ${error}`);
        return false;
    }
}
    ...
    try {
        ...
        if (!tabList) {
            if (!Config().debug) {
                ...
                ChatLib.chat(`&cNoNameAddons &8» &rFailed to get Crypt amount`);
                return 0;
            }
        }
        ...
        if (!Config().debug) { 
            ...
            ChatLib.chat(`&cNoNameAddons &8» &rFailed to get Crypt amount ${error}`);
            return 0;
        }
    }
}
